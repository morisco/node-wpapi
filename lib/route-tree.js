...
function reduceRouteComponents( routeObj, topLevel, parentLevel, component, idx, components ) {
  ...
  var myGroupPattern = groupPattern; 
  if (myGroupPattern !== null) { 
    myGroupPattern = myGroupPattern.replace("(?","\\(?"); 
  } 
  // console.log(myGroupPattern); 
  const groupPatternRE = myGroupPattern === '' ? 
  ...
  new RegExp( myGroupPattern ? '^' + myGroupPattern + '$' : component, 'i' ); 
